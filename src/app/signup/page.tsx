...
export default function SignUp() {
  ...
  const handleUsernameValidation = debounce(async (value: any) => {
    ...
    try {
      ...
      response.length === 0 ? setIsUsernameValid(1) : setNotValid();
    } catch (error: any) {
      console.log(error);
    }
  }, 600);
    ...
    <main className=" bg-[#F6F6F6] animate-fade">
      <div className="h-dvh w-[85%] xl:w-full xl:max-w-[25rem] mx-auto py-10 relative">
        ...
        >
          ...
          <button
            type="submit"
            className="bg-[#F26E56] text-[#F6F6F6] rounded-xl p-3.5 font-bold mt-8"
          >
            {isLoading ? <Loading style="w-[20px]" color="#F6F6F6" />  : "Continue"}
          </button>
        </form>
